...
import { Heart, Zap, Wrench, FlaskConical, AlertTriangle, ShieldCheck, Activity, Users, X, CheckCircle2, Terminal, RefreshCw, Trash2, Key, Play, Home, Trophy, Beaker, Grid3X3, Loader2, PlusCircle, LogIn, Timer, Wifi, WifiOff, Cpu, Battery, Database, Layers, Droplets, Thermometer, Wind, Fingerprint, Copy, Award } from 'lucide-react';
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const poll = setInterval(() => {
      if (appState === 'lobby' || appState === 'role-select') fetchNow();
    }, 2000);
  ...
  useEffect(() => {
    if (gameState?.last_start_at && (appState === 'lobby' || appState === 'role-select')) {
      setAppState('playing');
    }
  }, [gameState?.last_start_at, appState]);

  // --- COUNTDOWN LOGIC ---
  useEffect(() => {
    if (!gameState || appState !== 'lobby') return;
    const bothIn = gameState.engineer_taken && gameState.pharmacist_taken;
    if (bothIn && !gameState.last_start_at && countdown === null) {
      setCountdown(3);
    }
  }, [gameState, countdown, appState]);
    ...
    <div className="relative h-screen bg-cyber-black text-white overflow-hidden font-sans select-none">
      ...
      {currentHealth < 30 && appState === 'playing' && (
        <motion.div 
          ...
          className="fixed inset-0 pointer-events-none z-[60] shadow-[inset_0_0_200px_rgba(239,68,68,0.6)]"
        />
      )}
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="relative z-10 flex flex-col h-full">
      <div className="absolute top-0 left-0 right-0 h-1 bg-white/5 z-50">
        <motion.div className="h-full bg-cyber-accent shadow-[0_0_10px_#22d3ee]" animate={{ width: `${(time/GAME_DURATION)*100}%` }} transition={{ ease: "linear" }} />
      </div>
...
function TaskModal({ task, onComplete, onClose, role, updateDB }) {
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/95 backdrop-blur-3xl">
      <div className="w-full max-w-sm rounded-[45px] p-12 relative border border-white/10 bg-white/[0.02]">
        <button onClick={onClose} className="absolute top-10 right-10 text-gray-700 hover:text-white"><X size={28}/></button>
        <h2 className="text-3xl font-black mb-10 italic uppercase tracking-tighter">{task.replace('_', ' ')}</h2>
        <div className="min-h-[250px] flex items-center justify-center bg-white/5 rounded-[40px] p-8 mb-4 border border-white/5">
          ...
          {!['grid', 'chem', 'reactor', 'gencode', 'terminal', 'sweep', 'wires', 'battery', 'sort', 'vent'].includes(task) && (<div className="text-center"><p className="text-[10px] text-gray-500 mb-6 uppercase">Sync Verification</p><button onClick={() => onComplete(8)} className="py-4 px-8 bg-white/10 rounded-xl font-black">SOLVE</button></div>)}
        </div>
      </div>
    </div>
  );
...
function GridPulseTask({ onComplete }) {
  ...
  return <div className="w-full text-center"><div className="text-[10px] text-gray-500 mb-4 font-mono">CLEANING: {count}/15</div><div className="grid grid-cols-4 gap-4">{cells.map((a,i)=><button key={i} onClick={()=>click(i)} className={`w-14 h-14 rounded-2xl border-2 transition-all ${a?'bg-red-500 border-red-400 shadow-[0_0_20px_#ef4444]':'bg-white/5 border-white/5'}`}/>)}</div></div>;
}
function SortTask({ onComplete }) {
  ...
  return <div className="flex flex-col gap-6 items-center"><p className="text-[10px] text-gray-500 uppercase tracking-widest">Pick Cyan Samples</p><div className="flex gap-4 flex-wrap justify-center">{items.map((color, i) => <button key={i} onClick={() => click(color, i)} className={`w-14 h-14 rounded-2xl ${color === 'cyan' ? 'bg-cyan-400 shadow-[0_0_15px_#22d3ee]' : 'bg-pink-500'}`} />)}</div></div>;
}
function WiresTask({ onComplete }) {
  ...
  return <div className="flex flex-col gap-4 items-center"><p className="text-[10px] text-gray-500 mb-2">Connect in Order: {order.join(' ')}</p><div className="flex gap-4">{[1, 2, 3, 4].map(v => <button key={v} onClick={() => click(v)} className={`w-14 h-14 rounded-full border-2 font-black ${currentIdx > order.indexOf(v) ? 'bg-green-500 border-green-400' : 'bg-white/5 border-white/10'}`}>{v}</button>)}</div></div>;
}
function BatteryTask({ onComplete }) {
  ...
  return <div className="text-center w-full"><div className="h-10 w-full bg-white/5 rounded-2xl mb-8 overflow-hidden"><motion.div className="h-full bg-yellow-500" animate={{ width: `${charge}%` }} /></div><button onMouseDown={() => setHolding(true)} onMouseUp={() => setHolding(false)} onTouchStart={() => setHolding(true)} onTouchEnd={() => setHolding(false)} className="px-12 py-6 bg-white/10 rounded-3xl font-black text-xs uppercase tracking-widest active:scale-95 transition-all">Charge</button></div>;
}
function VentTask({ onComplete }) {
  ...
  return <div className="text-center w-full"><div className={`text-4xl font-black font-mono mb-8 ${pressure > 80 ? 'text-red-500 animate-pulse' : 'text-white'}`}>{pressure}%</div><button onClick={vent} className="w-full py-5 bg-white/5 border-2 border-white/10 rounded-2xl font-black uppercase text-xs">Vent</button></div>;
}
function ChemistryTask({ onComplete }) {
  ...
  return <div className="text-center w-full"><p className="text-[10px] text-gray-500 mb-4 uppercase">Target: {target}mg</p><div className="text-7xl font-black mb-12 font-mono">{cur}</div><div className="flex gap-4 justify-center">{[3,7,11].map(v=><button key={v} onClick={()=>add(v)} className="w-16 h-16 bg-white/10 border-2 border-pharmacist/40 rounded-3xl text-pharmacist font-black text-xl">+{v}</button>)}</div></div>;
}
...
function GenCodeTask({ onComplete, updateDB }) {
  ...
  return <div className="text-center w-full"><div className="text-7xl font-black font-mono mb-12 text-engineer">{code}</div><div className="flex flex-col gap-4"><button onClick={gen} className="w-full py-5 bg-engineer text-black font-black rounded-2xl shadow-lg">New Code</button><button onClick={() => onComplete()} className="w-full py-3 bg-white/5 text-[10px] font-bold text-gray-400 uppercase tracking-widest">Done</button></div></div>;
}
function TerminalTask({ onComplete }) {
  const [inp, setInp] = useState('');
  return <div className="text-center w-full"><input type="text" maxLength={4} value={inp} onChange={(e) => { setInp(e.target.value); if(e.target.value.length === 4) onComplete(); }} className="w-full bg-white/5 border-2 border-pharmacist/20 rounded-3xl py-6 text-center text-6xl font-black font-mono text-pharmacist outline-none focus:border-pharmacist" placeholder="----" /></div>;
}
function SweepTask({ onComplete }) {
  const [trash, setTrash] = useState([...Array(6)].map((_, i) => ({ id: i, x: Math.random()*80+10, y: Math.random()*60+10 })));
  const rem = (id) => { setTrash(t => { const n = t.filter(x => x.id !== id); if (n.length === 0) onComplete(); return n; }); };
  return <div className="relative w-full h-56">{trash.map(t => <motion.button key={t.id} drag dragConstraints={{ left: 0, right: 0, top: 0, bottom: 0 }} onDragEnd={() => rem(t.id)} className="absolute p-4 bg-red-500/20 border-2 border-red-500 text-red-500 rounded-3xl"><Trash2 size={24} /></motion.button>)}</div>;
}
